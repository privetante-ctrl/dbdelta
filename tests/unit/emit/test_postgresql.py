...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                "DROP DEFAULT",
                "DO $dbdelta$ BEGIN EXECUTE format('DROP SEQUENCE %s', "
                "pg_get_serial_sequence('\"t\"', 'a')); END $dbdelta$",
            ],
        ),
        (
            "serial",
...
def test_dollar_quotes_cannot_be_ended_by_names(migrate: Migrate) -> None:
    statements = sql(
        migrate(
            'CREATE TABLE "a$dbdelta$" (a serial)', 'CREATE TABLE "a$dbdelta$" (a int NOT NULL)'
        )
    )

    assert statements[1].startswith("DO $dbdelta1$ BEGIN")
    assert statements[1].endswith("END $dbdelta1$")
